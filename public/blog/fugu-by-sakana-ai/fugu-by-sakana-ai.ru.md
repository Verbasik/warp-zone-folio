...
Показать одну weight matrix как систему «фиксированные направления + обучаемые регуляторы громкости»: $(U)$ и $(V)$ задают направления, $(\Sigma)$ меняет их вклад. Рядом — сравнение полного fine-tuning и обучения нескольких диагональных коэффициентов.
...
**Важно:** logits и вероятности полностью иллюстративны. Это не реальные routing logs Fugu; открытых per-query traces в предоставленных научных документах нет.
