...
Но переносить этот результат на Fugu без оговорок нельзя. Fugu использует сходный optimisation principle, однако его backbone, training budget, pool и production trajectories не раскрыты полностью. Установленный факт — применение sep-CMA-ES; объяснение её преимуществ частично опирается на TRINITY.
